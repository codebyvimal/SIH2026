...
from backend.app.services.recommendation.logic import (
    COURSES,
    build_index,
    llm_rerank,
    recommend,
...
def test_llm_rerank_structure(monkeypatch):
    ...
    candidates = [
        (
            IgotCourse(
                course_id='c1', title='Python for Data Analysis', provider='iGOT', duration_hours=10
            ),
            0.85,
        ),
        (
            IgotCourse(
                course_id='c2', title='Applied Statistics', provider='NSSTA', duration_hours=20
            ),
            0.70,
        ),
    ]
    ...
    mock_output = RecommendationOutput(
        recommended=[
            RecommendedCourse(
                ...
                relevance=0.91,
                why='Directly relevant to python gap.',
            ),
        ]
    )
